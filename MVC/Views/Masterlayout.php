<?php
// Bảo vệ trang: nếu chưa đăng nhập thì chuyển về trang đăng nhập
if (session_status() === PHP_SESSION_NONE) session_start();
if (empty($_SESSION['username'])) {
    header("Location: /websitebanhangtaphoa/dangnhap_kh_c/Get_data");
    exit();
}
?>
<!DOCTYPE html>
        ...
        <div class="container">
            <div class="header-content">
                ...
                <div class="search-bar">
                    <form method="get" action="/websitebanhangtaphoa/home_menu/timkiem" style="display:inline;" onsubmit="return searchNotEmpty();">
                ...
                <div class="header-actions">
                    ...
                    <button class="header-btn" onclick="window.location.href='/websitebanhangtaphoa/dangnhap_kh_c/thongtincanhan'">👤 Tài khoản</button>
                    <button class="header-btn" onclick="window.location.href='/websitebanhangtaphoa/dangnhap_kh_c/logout'; return false;">Đăng xuất</button>
                </div>
            </div>
        </div>
    </header>
    ...
    <nav class="nav">
        <div class="container">
            <div class="nav-content">
                <div class="nav-menu">
                    <a href="/websitebanhangtaphoa/home_menu" class="nav-item">Tất cả</a>
                        ...
                        <select class="category-select" onchange="if(this.value) window.location.href=this.value;">
                            ...
                            <?php if (isset($data['dsDanhMuc']) && is_array($data['dsDanhMuc'])): ?>
                                <?php foreach ($data['dsDanhMuc'] as $dm): ?>
                                    <option value="/websitebanhangtaphoa/home_menu/locdanhmuc/<?= $dm['MaDM'] ?>"
        ...
        <div class="container">
            ...
            <section class="products-section">
                <?php if (isset($data['notification'])): ?>
                    <div class="search-notification" style="background: #ff6b6b; color: white; padding: 10px; border-radius: 8px; margin-bottom: 20px; text-align: center;">
                        <?= htmlspecialchars($data['notification']) ?>
                    </div>
                <?php endif; ?>
                <h2>
                    🏪
